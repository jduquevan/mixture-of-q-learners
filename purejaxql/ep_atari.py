...
def orchestrate_ep_train(config):
    # ---- assert ----
    save_config(config)
    
    rng = jax.random.PRNGKey(config["SEED"])
    initialization_rng, train_rng = jax.random.split(rng, 2)
    env, init_obs, init_env_state = initialize_env(config)
    network = QNetwork(action_dim=env.single_action_space.n, norm_type=config["NORM_TYPE"], norm_input=config.get("NORM_INPUT", False))
    agent_train_states, lr_function, eps_scheduler, buffer, buffer_index, buffer_filled = initialize_agent(config, env, initialization_rng, network) 

    for i in range(config["ROUNDS"]):
        print(f"Training Round {i} ------------------------------")

        def train_one_round_fn(train_states, rng, obs, env_state, buffer, buffer_index, buffer_filled):
            NUM_AGENTS = 1
            outs = train(env, obs, env_state, train_states, network, eps_scheduler,
                         buffer, buffer_index, buffer_filled, config["BUFFER_SIZE"], config["NUM_STEPS"],
                         NUM_AGENTS, config["NUM_ENVS"], config["NUM_TEST_ENVS"],
                         config["NUM_UPDATES"], config["NUM_EPOCHS"], config["NUM_MINIBATCHES"],
                         config.get("TEST_DURING_TRAINING", False), config["WANDB_MODE"], config["REW_SCALE"], config["GAMMA"],
                         config["LAMBDA"], lr_function, rng, config["NUM_ENVS"], log_prefix='big')
            ...
            return agent_train_states, (obs, env_state), test_metrics, _rng, buffer, buffer_index, buffer_filled
            
            
        jitted_train_one_round = jax.jit(train_one_round_fn)
        
        # train big agent one round
        agent_train_states, (obs, env_state), test_metrics, _rng, buffer, buffer_index, buffer_filled = jitted_train_one_round(
            agent_train_states, train_rng, init_obs, init_env_state, buffer, buffer_index, buffer_filled)
            
        # ---- save the big agent checkpoint ----
        updates = agent_train_states.n_updates[0]
        if updates % config["SAVE_CHECKPOINT_EVERY"] == 0:
            print(f"Saving checkpoint at step {updates}")
            save_big_checkpoint(config, agent_train_states, updates)
            
    outs = {
        'runner_state': (agent_train_states, (obs, env_state), test_metrics, _rng, buffer, buffer_index, buffer_filled),
...
def initialize_env(config):
    num_envs = config["NUM_ENVS"]
    test_envs = config["NUM_TEST_ENVS"]
    total_envs = (num_envs + test_envs)
    env = make_env(total_envs, config)
    init_obs, env_state = env.reset()
    return env, init_obs, env_state

def initialize_agent(config, env, rng, network):
    
    eps_scheduler = optax.linear_schedule(config["EPS_START"], config["EPS_FINISH"], (config["NUM_UPDATES_DECAY"]))
    
    def pass_rng_through_scheduler_decorator(eps_scheduler):
            def fn(n_updates, _rng):
                return eps_scheduler(n_updates)
            return fn
        
    def get_depth_exploration_eps(eps_scheduler, activation_prob, max_exploration_eps):
        def fn(n_updates, _rng):
            coin_toss = jax.random.bernoulli(_rng, activation_prob)
            original_eps = eps_scheduler(n_updates)
            exploration_eps = jax.random.uniform(_rng, (), minval=0, maxval=max_exploration_eps)
            return jnp.where(coin_toss, exploration_eps, original_eps)
        return fn
    
    def get_envid_pass_through_scheduler_decorator(eps_scheduler):
        def fn(n_updates, _rng, env_id):
            return eps_scheduler(n_updates, _rng)
        return fn
    
    def get_env_wise_exploration_eps(eps_scheduler, exploit_ratio, max_exploration_eps):
        rng_eps = rax.fold_in(rng, 897)
        def fn(n_updates, _rng, env_id):
            rng_env = rax.fold_in(rng_eps, env_id)
            coin_toss = jax.random.bernoulli(rng_env, exploit_ratio)
            original_eps = eps_scheduler(n_updates, _rng)
            exploration_eps = jax.random.uniform(rng_env, (), minval=0, maxval=max_exploration_eps)
            return jnp.where(coin_toss, original_eps, exploration_eps)
        return fn
    
    def get_env_wise_exploration_eps_periodic(eps_scheduler, min_exploit_ratio, max_exploit_ratio, max_exploration_eps, period):
        rng_eps = rax.fold_in(rng, 897)
        def fn(n_updates, _rng, env_id):
            rng_env = rax.fold_in(rng_eps, env_id)
            threshold = jax.random.uniform(rng_env, (), minval=0, maxval=1)
            original_eps = eps_scheduler(n_updates, _rng)
            exploration_eps = jax.random.uniform(rng_env, (), minval=0, maxval=max_exploration_eps)
            exploit_ratio = jnp.where(n_updates // period % 2 == 1, min_exploit_ratio, max_exploit_ratio)
            return jnp.where(threshold < exploit_ratio, original_eps, exploration_eps)           
        return fn
       
        
    if config["DEPTH_EXPLORATION_EPS_SCHEDULER"] == "depth_exploration_0.1_U0.3":
        eps_scheduler = get_depth_exploration_eps(eps_scheduler, activation_prob=0.1, max_exploration_eps=0.3)
    elif config["DEPTH_EXPLORATION_EPS_SCHEDULER"] == "depth_exploration_0.1_U0.05":
        eps_scheduler = get_depth_exploration_eps(eps_scheduler, activation_prob=0.1, max_exploration_eps=0.05)
    elif config["DEPTH_EXPLORATION_EPS_SCHEDULER"] == "disabled":
        # change so it accepts an rng and the interface is the same as the one above
        eps_scheduler = pass_rng_through_scheduler_decorator(eps_scheduler)
    else:
        raise ValueError(f"Invalid exploration epsilon scheduler: {config['EXPLORATION_EPS_SCHEDULER']}")
    
    if config["ENV_WISE_EXPLORATION_EPS_SCHEDULER"] == "envwise50D50R_U0.3":
        eps_scheduler = get_env_wise_exploration_eps(eps_scheduler, exploit_ratio=0.5, max_exploration_eps=0.3)
    elif config["ENV_WISE_EXPLORATION_EPS_SCHEDULER"] == "envwise50D50R_U0.05":
        eps_scheduler = get_env_wise_exploration_eps(eps_scheduler, exploit_ratio=0.5, max_exploration_eps=0.05)
    elif config["ENV_WISE_EXPLORATION_EPS_SCHEDULER"] == "envwise80D20R_U0.05":
        eps_scheduler = get_env_wise_exploration_eps(eps_scheduler, exploit_ratio=0.8, max_exploration_eps=0.05)
    elif config["ENV_WISE_EXPLORATION_EPS_SCHEDULER"] == "envwise_periodic_1D0R_0.5D0.5R_U0.05":
        eps_scheduler = get_env_wise_exploration_eps_periodic(eps_scheduler, min_exploit_ratio=0.0, max_exploit_ratio=0.5, max_exploration_eps=0.05, period=config["EXPLORATION_PERIOD"])
    elif config["ENV_WISE_EXPLORATION_EPS_SCHEDULER"] == "disabled":
        eps_scheduler = get_envid_pass_through_scheduler_decorator(eps_scheduler)
    else:
        raise ValueError(f"Invalid exploration epsilon scheduler: {config['ENV_WISE_EXPLORATION_EPS_SCHEDULER']}")
    ...
    agent_train_states, lr_function = initialize_agents(env, 
                                                        ...
                                                        config["NUM_UPDATES_DECAY"], #TODO: remove the lr decay in general, but this one just works for mid rounds.
    ...
    return agent_train_states, lr_function, eps_scheduler, buffer, buffer_index, buffer_filled
...
def train(
    ...
    original_seed = rng[0]
    assert NUM_AGENTS == 1, "This code is only for a single agent"
    ...
    def _update_step(runner_state, update_step):
        ...
        def _step_env(carry, _):
            ...
            def get_eps_per_agent(n_updates, _rngs):
                env_ids = jnp.arange(NUM_ENVS + TEST_ENVS)
                eps = jax.vmap(lambda _rng, _env_id: eps_scheduler(n_updates, _rng, _env_id))(_rngs, env_ids)
        ...
        for i in range(NUM_AGENTS):
            random_env_id = jax.random.randint(rng, (), 0, NUM_ENVS)
            eps_val = eps_scheduler(agent_train_states.n_updates[i], rng, random_env_id) # just pass some rng, we will get a distribution in plots =) 
...
def single_run(config):
    ...
    # --- train ---
    rng = jax.random.PRNGKey(config["SEED"])    
    outs = orchestrate_ep_train(config)
